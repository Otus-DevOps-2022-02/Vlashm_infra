...
- Создан файл конфигурации *Vagrantfile*
- Доработаны роли *app* и *db*:
    - Таски вынесены в отдельные файлы
    - параметризирован пользователь в роли *app*
...
- Добавлены необходимые зависимости в *ansible/requirements.txt*
- Командой `molecule init scenario -r db -d vagrant` создан сценарий для тестирования роли *db*
- Создан файл *db/molecule/default/tests/test_default.py* с тестами для проверки роли *db*
- Внесены изменения в плейбук *db/molecule/converge.yml* для выполнения таск из-под суперпользователя.
- Добавлен тест для проверки доступности БД на порте 27017
- Изменены плэйбуки *packer_db.yml* и *packer_app.yml* на использование ролей
- Внесены изменения в *packer/app.json* и *packer/db.json*, для того, чтобы роли были найдены

### Задание со *

- Роль *db* вынесена в отдельный репозиторий https://github.com/Vlashm/ansible_role_db и добавлено подключение роли в окружения *stage* *prod* через *requirements.yml*.
- Добавлен прогон тестов на AWS, но, к сожалению подключить созданный репозиторий к *TravisCI* не удалось, поскольку требуется привязка банковской карты.
